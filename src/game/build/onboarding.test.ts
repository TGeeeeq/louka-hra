...
const STEP_TX = [30, 33, 35, 36, 38, 41, 44, 47, 49];
const STEP_TY = 30;
...
describe("guided tutorial placement", () => {
  ...
  it("building a pen bumps that group's population to full", () => {
    ...
    for (let i = 0; i < idxKurnik; i++) {
      s = reducer(s, { type: "PLACE_STRUCTURE", defId: TUTORIAL_STEPS[i].buildingId, tx: STEP_TX[i], ty: STEP_TY });
    }
    const before = s.population.drubez;
    s = reducer(s, { type: "PLACE_STRUCTURE", defId: "kurnik", tx: STEP_TX[idxKurnik], ty: STEP_TY });
