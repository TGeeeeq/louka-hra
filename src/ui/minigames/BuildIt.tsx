...
// Krátká stavební minihra — zatloukej, když je značka v zeleném.
const NEED = 3;
const MAX_TRY = 8;
const ZONE_HALF = 0.13; // poloviční šířka zásahové zóny kolem středu

export function BuildIt({ label, onWin, onClose }: { label: string; onWin: () => void; onClose: () => void }) {
  ...
  const hammer = () => {
    ...
    if (inZone) {
      ...
      sound.build();
      speedRef.current = Math.min(0.036, speedRef.current + 0.004);
    } else {
      sound.error();
    }
  ...
  useEffect(() => {
    const k = (e: KeyboardEvent) => {
      if (e.code === "Space" || e.code === "Enter") { e.preventDefault(); hammer(); }
    };
  ...
  const reset = () => {
    posRef.current = 0; dirRef.current = 1; speedRef.current = 0.015; hitsRef.current = 0; triesRef.current = 0;
  ...
  if (result) {
    ...
    return (
      <div className="mg">
        <h3>{win ? "🔨 Postaveno!" : "😅 Ještě ne."}</h3>
        <p className="mg-result">
          {win ? `${label} stojí jak zákon káže. Paráda!` : "Kladivo šlo vedle. Chytni ten rytmus a zkus to znovu."}
        </p>
  ...
  return (
    <div className="mg">
      <p className="mg-q">Stavíš: <b>{label}</b></p>
      <p className="mg-q">Zatluč, když je značka v zeleném! (mezerník nebo tlačítko)</p>
      ...
      <button className="big-btn mg-next" onClick={hammer}>🔨 Zatlouct!</button>
    </div>
  );
}
